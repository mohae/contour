...
func NewITests() []*iTest {
	...
	tests[3] =   &iTest{"configbool", "configbool", true, "", "configbool: setting is not a flag. Only flags can be overridden"}
	tests[4] =   &iTest{"configint", "configint", 43, "", "configint: setting is not a flag. Only flags can be overridden"}
	tests[5] =   &iTest{"configstring", "configstring", "frood", "", "configstring: setting is not a flag. Only flags can be overridden"}
...
func TestOverride(t *testing.T) {
	...
	for _, test := range tests {
		Convey("Given a test and a testConfig", t, func() {			
			Convey(test.name + ": Overridding " + test.key, func() {
				err := testConfig.Override(test.key, test.value)
